...
const presets = [
  {
    ...
    claudeWriteType: '5m', cacheWritesPerDay: 0, geminiEndpointType: 'global',
  },
  {
    name: '\u672C\u756A\u904B\u7528',
...
function encodeState(full) {
  const parts = [];
  const tab = state.activeTab;
  if (full) parts.push('t=' + (tab === 'simulator' ? 'sim' : tab === 'scenario' ? 'sc' : 'mem'));
  else if (tab === 'scenario') parts.push('t=sc');
  else if (tab === 'memory') parts.push('t=mem');
  if (full || state.exchangeRate !== defaultExchangeRate) parts.push('fx=' + state.exchangeRate);
  ...
  if (tab === 'simulator' || tab === 'scenario') {
    ...
    if (tab === 'scenario' && (full || state.activeScenario !== 0)) parts.push('si=' + state.activeScenario);
    simKeyMap.forEach(function(kv) {
      if (full || p[kv[1]] !== d[kv[1]]) parts.push(kv[0] + '=' + p[kv[1]]);
    });
    if (full || p.claudeWriteType !== d.claudeWriteType) parts.push('wt=' + p.claudeWriteType);
    if (full || p.geminiEndpointType !== d.geminiEndpointType) parts.push('gep=' + p.geminiEndpointType);
  } else if (tab === 'memory') {
    const m = state.mem, d = defaultMem;
    memKeyMap.forEach(function(kv) {
      if (full || m[kv[1]] !== d[kv[1]]) parts.push(kv[0] + '=' + m[kv[1]]);
    });
    if (full || m.ltmRetrievalTiming !== d.ltmRetrievalTiming) parts.push('mT=' + m.ltmRetrievalTiming);
    if (full || m.useOverrideStrategy !== d.useOverrideStrategy) parts.push('mO=' + (m.useOverrideStrategy ? 1 : 0));
    if (full || m.extractionEnabled !== d.extractionEnabled) parts.push('mX=' + (m.extractionEnabled ? 1 : 0));
  }
  return parts.join('&');
}
...
function decodeState() {
  ...
  else if (params.t === 'sim') state.activeTab = 'simulator';
  ...
  if (state.activeTab === 'simulator' || state.activeTab === 'scenario') {
    ...
    if (params.gep === 'global' || params.gep === 'nonglobal') p.geminiEndpointType = params.gep;
...
function updateShareUrl() {
  ...
  // Leave the URL alone if it already represents the current state
  // (covers both auto-sync diff form and pasted full-snapshot form)
  if (cur === encodeState(false) || cur === encodeState(true)) return;
  const enc = encodeState(false);
...
function computeResults(p) {
  ...
  const results = Object.entries(pricingData).map(([key, m]) => {
    ...
    const st = mp.isGemini ? mp.cM * mAdj.storage * totalHours : 0;
    const rd = mp.crM * mAdj.cachedInput * totalReqs;
    const pr = mp.pM * mAdj.input * totalReqs;
    const ou = mp.oM * mAdj.output * totalReqs;
    const rs = mp.rsM * mAdj.output * totalReqs;
    const tot = cwCost + st + rd + pr + ou + rs;
    const pReq = totalReqs > 0 ? tot / totalReqs : 0;
    const noC = (mp.crM + mp.cwM + mp.pM) * mAdj.input * totalReqs + ou + rs;
    return {
      ...mAdj, id: key, isClaude: mp.isClaude, isOpenAI: mp.isOpenAI, isNova: mp.isNova, isGemini: mp.isGemini,
...
function computeTimeSeries(p) {
  ...
  return Object.entries(pricingData).map(([key, m]) => {
    ...
    const dRead = mp.crM * mAdj.cachedInput * dailyReqs;
    const dPrompt = mp.pM * mAdj.input * dailyReqs;
    const dOutput = mp.oM * mAdj.output * dailyReqs;
    const dReasoning = mp.rsM * mAdj.output * dailyReqs;
    const daily = dCwWrite + dStorage + dRead + dPrompt + dOutput + dReasoning;
    const pts = [];
    for (let d = 0; d <= p.testDays; d++) pts.push({ day: d, usd: daily * d });
    return { ...mAdj, id: key, points: pts, daily, total: daily * p.testDays };
  });
}
  ...
  return '<div class="mt-4 space-y-2">' +
    ...
    '<div class="flex flex-wrap items-center gap-3 px-3 py-2 border border-slate-100 rounded-lg">' +
      ...
      '<div class="flex gap-1">' +
        '<button data-prefix="' + prefix + '" data-gep="global" class="px-2 py-0.5 text-[11px] rounded border ' + (p.geminiEndpointType === 'global' ? 'bg-slate-800 text-white border-slate-800' : 'border-slate-200 text-slate-500') + '">Global</button>' +
        '<button data-prefix="' + prefix + '" data-gep="nonglobal" class="px-2 py-0.5 text-[11px] rounded border ' + (p.geminiEndpointType === 'nonglobal' ? 'bg-slate-800 text-white border-slate-800' : 'border-slate-200 text-slate-500') + '">Non-global</button>' +
      '</div>' +
...
function downloadExcel() {
  ...
  platforms.forEach(function(plat) {
    ...
    mods.forEach(function(o, i) {
      var c = cols[i];
      var mAdj = adjustModelForEndpoint(o.m, p, o.key);
      n(c+'4', mAdj.input);
      if (plat.useWriteFormula) {
        fm(c+'5', 'IF('+P+'15=1,'+mAdj.cacheWrite1h+','+mAdj.cacheWrite5m+')');
      } else {
        n(c+'5', mAdj.input);
      }
      n(c+'6', mAdj.cachedInput);
      n(c+'7', mAdj.output);
      n(c+'8', mAdj.storage);
      n(c+'9', plat.wpd());
    });
...
function render() {
  ...
  app.querySelectorAll('button[data-gep]').forEach(el => {
    el.addEventListener('click', () => {
      const target = el.dataset.prefix === 'sim' ? state.sim : state.scenarios[state.activeScenario];
      target.geminiEndpointType = el.dataset.gep;
      render();
    });
  });
  // Scenario buttons
