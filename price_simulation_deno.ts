...
function render() {
  ...
  if (state.activeTab === 'simulator') {
    ...
    // Cost breakdown — with formula column
    var ltmPrice = m.useOverrideStrategy ? 0.25 : 0.75;
    var exIn = m.extractionInputTokens || 0, exOut = m.extractionOutputTokens || 0;
    var exModel = m.extractionModel && pricingData[m.extractionModel] ? pricingData[m.extractionModel] : null;
    var exInP = exModel ? exModel.input : 0, exOutP = exModel ? exModel.output : 0;
    html += '<section class="mb-8"><h2 class="text-sm font-semibold text-slate-700 mb-3">\u30B3\u30B9\u30C8\u5185\u8A33 (\u6708\u984D)</h2>' +
      ...
      '<tbody class="font-mono text-xs">' +
        '<tr class="border-t border-slate-100"><td class="px-4 py-2 text-slate-600 font-sans">STM \u30A4\u30D9\u30F3\u30C8</td><td class="px-4 py-2 text-[10px] text-slate-400">' + mr.totalEvents.toLocaleString() + ' \u00D7 $0.25/1K</td><td class="px-4 py-2 text-right">$' + fU(mr.stmCost) + '</td><td class="px-4 py-2 text-right">&yen;' + fJ(mr.stmCost) + '</td><td class="px-4 py-2 text-right text-slate-400">' + (mr.totalCost > 0 ? (mr.stmCost / mr.totalCost * 100).toFixed(1) : '0') + '%</td></tr>' +
        '<tr class="border-t border-slate-100"><td class="px-4 py-2 text-slate-600 font-sans">LTM \u30B9\u30C8\u30EC\u30FC\u30B8' + (m.useOverrideStrategy ? ' (Override)' : ' (Default)') + '</td><td class="px-4 py-2 text-[10px] text-slate-400">' + mr.totalRecords.toLocaleString() + ' \u00D7 $' + ltmPrice.toFixed(2) + '/1K</td><td class="px-4 py-2 text-right">$' + fU(mr.ltmStorageCost) + '</td><td class="px-4 py-2 text-right">&yen;' + fJ(mr.ltmStorageCost) + '</td><td class="px-4 py-2 text-right text-slate-400">' + (mr.totalCost > 0 ? (mr.ltmStorageCost / mr.totalCost * 100).toFixed(1) : '0') + '%</td></tr>' +
        '<tr class="border-t border-slate-100"><td class="px-4 py-2 text-slate-600 font-sans">LTM \u691C\u7D22</td><td class="px-4 py-2 text-[10px] text-slate-400">' + mr.totalRetrievals.toLocaleString() + ' \u00D7 $0.50/1K</td><td class="px-4 py-2 text-right">$' + fU(mr.ltmRetrievalCost) + '</td><td class="px-4 py-2 text-right">&yen;' + fJ(mr.ltmRetrievalCost) + '</td><td class="px-4 py-2 text-right text-slate-400">' + (mr.totalCost > 0 ? (mr.ltmRetrievalCost / mr.totalCost * 100).toFixed(1) : '0') + '%</td></tr>' +
        (m.useOverrideStrategy && mr.extractionCost > 0 ? '<tr class="border-t border-slate-100"><td class="px-4 py-2 text-slate-600 font-sans">LLM \u62BD\u51FA <span class="text-[10px] text-slate-400">(' + (exModel ? exModel.name : '') + ')</span></td><td class="px-4 py-2 text-[10px] text-slate-400">(' + exIn + '/' + exOut + 'tok \u00D7 $' + exInP + '/$' + exOutP + '/MTok) \u00D7 ' + mr.totalRoundtrips.toLocaleString() + 'req</td><td class="px-4 py-2 text-right">$' + fU(mr.extractionCost) + '</td><td class="px-4 py-2 text-right">&yen;' + fJ(mr.extractionCost) + '</td><td class="px-4 py-2 text-right text-slate-400">' + (mr.totalCost > 0 ? (mr.extractionCost / mr.totalCost * 100).toFixed(1) : '0') + '%</td></tr>' : '') +
        ...
        '<tr class="border-t border-slate-200"><td class="px-4 py-2 font-sans text-slate-500">1\u30EA\u30AF\u30A8\u30B9\u30C8\u5358\u4FA1</td><td class="px-4 py-2 text-[10px] text-slate-400">$' + fU(mr.totalCost) + ' / ' + mr.totalRoundtrips.toLocaleString() + '</td><td class="px-4 py-2 text-right">$' + mr.perRequest.toFixed(5) + '</td><td class="px-4 py-2 text-right">&yen;' + fJ2(mr.perRequest) + '</td><td></td></tr>' +
        '<tr class="border-t border-slate-100"><td class="px-4 py-2 font-sans text-slate-500">\u30BB\u30C3\u30B7\u30E7\u30F3\u5358\u4FA1</td><td class="px-4 py-2 text-[10px] text-slate-400">$' + fU(mr.totalCost) + ' / ' + mr.totalSessions.toLocaleString() + '</td><td class="px-4 py-2 text-right">$' + mr.perSession.toFixed(4) + '</td><td class="px-4 py-2 text-right">&yen;' + fJ2(mr.perSession) + '</td><td></td></tr>' +
        '<tr class="border-t border-slate-100"><td class="px-4 py-2 font-sans text-slate-500">\u30E6\u30FC\u30B6\u30FC\u6708\u984D</td><td class="px-4 py-2 text-[10px] text-slate-400">$' + fU(mr.totalCost) + ' / ' + m.userCount.toLocaleString() + '</td><td class="px-4 py-2 text-right">$' + mr.perUserMonth.toFixed(4) + '</td><td class="px-4 py-2 text-right">&yen;' + fJ2(mr.perUserMonth) + '</td><td></td></tr>' +
        '<tr class="border-t border-slate-100"><td class="px-4 py-2 font-sans text-slate-500">\u30E6\u30FC\u30B6\u30FC\u5E74\u984D</td><td class="px-4 py-2 text-[10px] text-slate-400">$' + mr.perUserMonth.toFixed(4) + ' \u00D7 12</td><td class="px-4 py-2 text-right">$' + mr.perUserYear.toFixed(4) + '</td><td class="px-4 py-2 text-right">&yen;' + fJ2(mr.perUserYear) + '</td><td></td></tr>' +
      '</tbody></table></div></section>';
