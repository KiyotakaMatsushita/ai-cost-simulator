...
| 出力トークン/req (O) | 1リクエストあたりの可視出力トークン数 |
| 推論トークン/req (Rs) | 1リクエストあたりの推論/思考(thinking/reasoning)トークン数。output 単価で課金される不可視トークン |
...
5. **推論** - 推論/思考トークン（Rs × OutputPrice × TotalReqs）※output と同単価
6. **Storage** - 保管料（Geminiのみ、C × StoragePrice × Hours）
